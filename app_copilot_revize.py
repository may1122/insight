...
# ============================================================
st.set_page_config(
    page_title="AYÇA Insight V10.7.4 Premium Health SaaS",
    page_icon="💊",
    layout="wide",
...
st.markdown(
    ...
    /* AYÇA Branding - logo/header/footer */
    .ayca-brand-strip {
        display:flex;
        align-items:center;
    ...
    .ayca-brand-left {display:flex; align-items:center; gap:12px;}
    .ayca-brand-logo-img {display:block; object-fit:contain;}
    .ayca-brand-text-main {font-size:16px; font-weight:950; color:#0F172A; letter-spacing:-.2px;}
    .ayca-brand-text-sub {font-size:12px; font-weight:800; color:#64748B; margin-top:2px;}
    .ayca-brand-pill {
    ...
    .ayca-sidebar-brand img {max-width:145px; width:100%; height:auto; margin:0 auto 10px auto; display:block;}
    .ayca-sidebar-title {font-size:16px; font-weight:950; color:#0F172A; letter-spacing:-.2px;}
    .ayca-sidebar-sub {font-size:12px; color:#64748B; font-weight:800; line-height:1.35; margin-top:4px;}
    .ayca-sidebar-url {font-size:12px; color:#2563EB; font-weight:950; margin-top:10px;}
    .ayca-footer {
        margin:28px 0 8px 0;
        padding:14px 16px;
        text-align:center;
...
AYCA_APP_VERSION = "V10.7.4"
AYCA_WEBSITE_TEXT = "www.aycayazilim.com"
AYCA_WEBSITE_URL = "https://www.aycayazilim.com"


@st.cache_data(show_spinner=False)
def get_ayca_logo_data_uri() -> str:
    """Logo dosyasını güvenli şekilde base64'e çevirir. Dosya yoksa uygulama çökmez."""
    candidate_paths = []
    try:
        candidate_paths.append(Path(__file__).parent / "assets" / "ayca_logo.png")
    except Exception:
        pass
    candidate_paths += [
        Path("assets") / "ayca_logo.png",
        Path("ayca_logo.png"),
    ]
...
def render_ayca_logo_html(width: int = 148, extra_class: str = "") -> str:
    logo_uri = get_ayca_logo_data_uri()
    if logo_uri:
        return f'<img class="ayca-brand-logo-img {extra_class}" src="{logo_uri}" width="{int(width)}" alt="AYÇA Yazılım">'
    return '<div class="ayca-logo-fallback">AYÇA</div>'


def show_ayca_sidebar_brand():
    st.sidebar.markdown(
        f"""
        <div class="ayca-sidebar-brand">
...
def show_ayca_brand_strip():
    st.markdown(
        f"""
        <div class="ayca-brand-strip">
            <div class="ayca-brand-left">
                {render_ayca_logo_html(118)}
                <div>
                    <div class="ayca-brand-text-main">AYÇA Insight</div>
                    <div class="ayca-brand-text-sub">Premium Health SaaS · Eczane Yönetim Zekâsı</div>
                </div>
            </div>
            <div class="ayca-brand-pill">{AYCA_APP_VERSION} · {AYCA_WEBSITE_TEXT}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def show_ayca_footer():
    st.markdown(
        ...
        <div class="ayca-footer">
            Powered by <b>AYÇA Yazılım</b> · {AYCA_WEBSITE_TEXT}
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with left:
        st.markdown(
            ...
            <div class="login-clean-left">
                <div style="position:relative; z-index:2; margin-bottom:14px;">{render_ayca_logo_html(168)}</div>
...
show_ayca_sidebar_brand()
st.sidebar.caption("V10.7.4 Premium Health SaaS · Sağlık Barları + Hazır Sorular")
...
def copilot_answer(question: str, product_master: pd.DataFrame, current_stats: dict, reorder_df: pd.DataFrame,
                   ...
                   patient_loyalty: dict, doctor_intel: dict, payment_df: Optional[pd.DataFrame],
                   kurum_df: Optional[pd.DataFrame] = None) -> list[str]:
    """AYÇA Copilot hazır sorularına veri bazlı ve temkinli cevap üretir.

    Not: Bu bölüm serbest LLM değildir. Sadece uygulamada gerçekten hesaplanan alanlara dayanır.
    Bu nedenle soru listesi de mevcut veri kaynaklarıyla doğrulanabilir başlıklardan oluşur.
    """
    q_raw = str(question or "").strip()
    q = normalize_col_name(q_raw)
    answers: list[str] = []

    pm = product_master.copy() if product_master is not None else pd.DataFrame()
    reorder = reorder_df.copy() if reorder_df is not None else pd.DataFrame()
    dead = dead_df.copy() if dead_df is not None else pd.DataFrame()
    slow = slow_df.copy() if slow_df is not None else pd.DataFrame()
    kurum = kurum_df.copy() if kurum_df is not None else pd.DataFrame()

    def _top_rows(df: pd.DataFrame, sort_col: str, n: int = 3) -> pd.DataFrame:
    ...
    def _product_list(df: pd.DataFrame, sort_col: str = "satis_tutari", n: int = 3) -> str:
        rows = _top_rows(df, sort_col, n)
        if rows.empty or "urun" not in rows.columns:
            return "-"
        return ", ".join(rows["urun"].astype(str).head(n).tolist())

    # Risk setleri uygulamanın Risk Master eşleşmelerinden gelir.
    risk_flag_map = {
        "kirmizi": "kirmizi_recete_mi",
        "yesil": "yesil_recete_mi",
        "mor": "mor_recete_mi",
        "ek_izlem": "ek_izlem_mi",
        "kki": "kki_riskli_mi",
    }
    risk_sets = {}
    for key, col in risk_flag_map.items():
        if not pm.empty and col in pm.columns:
            risk_sets[key] = pm[pm[col].fillna(False)].copy()
        else:
            risk_sets[key] = pd.DataFrame()

    if "karlilik_durumu" in q or ("kar" in q and "urun" not in q and "sessiz" not in q):
        answers.append(f"Seçili dönemde brüt kâr {money_fmt(current_stats.get('kar', 0))}, brüt marj {pct_fmt(current_stats.get('marj', 0))}.")
        if current_stats.get("marj", 0) < 0.12:
            answers.append("Marj düşük görünüyor. İlk kontrol: düşük marjlı ama çok satan ürünler ve tahsilat açığı.")
        elif current_stats.get("marj", 0) < 0.15:
            answers.append("Marj izlenebilir seviyede; ancak kampanya, iskonto ve düşük kârlı hızlı ürünler kontrol edilmeli.")
        else:
            answers.append("Marj sağlıklı görünüyor. Öncelik bu marjı koruyarak stok devir hızını artırmak olmalı.")

    elif "en_cok_kar_getiren_urun" in q:
        profit = pm[pm.get("satilan_adet", 0) > 0].sort_values("kar_tutari", ascending=False) if not pm.empty and "kar_tutari" in pm.columns else pd.DataFrame()
        if not profit.empty:
            r = profit.iloc[0]
            answers.append(f"En çok kâr getiren ürün: {r.get('urun','-')} · {money_fmt(r.get('kar_tutari',0))}.")
            answers.append(f"Satılan adet: {num_fmt(r.get('satilan_adet',0),0)} · Ciro: {money_fmt(r.get('satis_tutari',0))}.")
        else:
            answers.append("Bu cevap için ürün bazında kâr tutarı verisi bulunmalı.")

    elif "sessiz_kar_kaybi" in q:
        silent = business_insights.get("silent_loss", pd.DataFrame()) if business_insights else pd.DataFrame()
        if silent is not None and not silent.empty:
            r = silent.iloc[0]
            answers.append(f"Sessiz kâr kaybı adayı: {r.get('urun','-')} · tahmini etki {money_fmt(r.get('tahmini_sessiz_kayip',0))}.")
            answers.append("Bu ürün çok satmasına rağmen marjı düşük olduğu için kârlılığı aşağı çekebilir.")
        else:
            answers.append("Mevcut veride belirgin sessiz kâr kaybı adayı görünmüyor.")

    elif "sermaye_baglayan" in q:
        answers.append(f"Stokta bağlı toplam sermaye yaklaşık {money_fmt(pm.get('stok_degeri', pd.Series(dtype=float)).sum())}.")
        capital = _top_rows(pm, "stok_degeri", 3)
        if not capital.empty and "urun" in capital.columns:
            answers.append("En çok sermaye bağlayan ilk ürünler: " + ", ".join(capital["urun"].astype(str).tolist()) + ".")

    elif "olu_stok" in q:
        if dead is not None and not dead.empty:
            r = dead.sort_values("stok_degeri", ascending=False).iloc[0] if "stok_degeri" in dead.columns else dead.iloc[0]
            answers.append(f"Ölü stokta ilk kontrol: {r.get('urun','-')} · stok değeri {money_fmt(r.get('stok_degeri',0))}.")
            answers.append("Aksiyon: raf görünürlüğü, kampanya, muadil yönlendirme veya tedarik kesme kontrol edilmeli.")
        else:
            answers.append("Seçili kriterlere göre ölü stok listesi boş görünüyor.")

    elif "yavas_stok" in q:
        if slow is not None and not slow.empty:
            answers.append(f"Yavaş stokta {len(slow)} ürün var. İlk kontrol: {_product_list(slow, 'stok_degeri', 3)}.")
        else:
            answers.append("Seçili kriterlere göre yavaş stok listesi boş görünüyor.")

    elif "genel_ilac_siparisi" in q or "acil_siparis" in q or "recete_geldikce" in q:
        if reorder is None or reorder.empty:
            answers.append("Bütçeli sipariş listesinde ürün görünmüyor.")
        elif "recete_geldikce" in q:
            if "recete_geldikce_al_mi" in pm.columns:
                rx_needed = pm[pm["recete_geldikce_al_mi"].fillna(False)].copy()
                answers.append(f"Reçete geldikçe alınacak pahalı/seyrek ürün sayısı: {len(rx_needed)}.")
                if not rx_needed.empty:
                    answers.append("İlk örnekler: " + _product_list(rx_needed, "birim_satis", 3) + ".")
            else:
                answers.append("Bu cevap için 'recete_geldikce_al_mi' segmenti hesaplanmış olmalı.")
        elif "acil_siparis" in q:
            if "siparis_segmenti" in reorder.columns:
                urgent = reorder[reorder["siparis_segmenti"].astype(str).str.contains("Acil", case=False, na=False)].copy()
            elif "acil_siparis_mi" in reorder.columns:
                urgent = reorder[reorder["acil_siparis_mi"].fillna(False)].copy()
            else:
                urgent = reorder.copy()
            answers.append(f"Acil sipariş kontrol listesinde {len(urgent)} ürün var.")
            if not urgent.empty:
                answers.append("İlk kontrol: " + _product_list(urgent, "siparis_tahmini_tutar", 3) + ".")
        else:
            answers.append(f"Genel sipariş listesinde {len(reorder)} ürün var; toplam öneri tutarı {money_fmt(reorder.get('siparis_tahmini_tutar', pd.Series(dtype=float)).sum())}.")
            answers.append("İlk kontrol: " + _product_list(reorder, "siparis_tahmini_tutar", 3) + ".")

    elif "kaybettigim_hastalar" in q:
        ...
        if lost is not None and not lost.empty:
            ...
            answers.append(f"Kayıp riski grubunda toplam {len(lost)} hasta görünüyor.")
        else:
            answers.append("Mevcut veride kayıp hasta adayı görünmüyor.")

    elif "vip_hastalar" in q or "hasta_sadakati" in q:
        summary = patient_loyalty.get("summary", {}) if patient_loyalty else {}
        ...
        if summary.get("kayip_riski", 0):
            answers.append("Öncelik: yüksek ciro bırakmış ama son dönemde görünmeyen hastaları geri kazanmak.")

    elif "en_cok_ciro_getiren_doktor" in q:
        doc = doctor_intel.get("doctor_kpi", pd.DataFrame()) if doctor_intel else pd.DataFrame()
        if doc is not None and not doc.empty:
            top = doc.sort_values("ciro", ascending=False).head(3)
            answers.append("En çok ciro getiren doktorlar: " + ", ".join(top["doktor_clean"].astype(str).tolist()) + ".")
            r = top.iloc[0]
            answers.append(f"İlk doktor: {r.get('doktor_clean','-')} · {money_fmt(r.get('ciro',0))}.")
        else:
            answers.append("Doktor analizi için satış hareketlerinde doktor alanı bulunmalı.")

    elif "doktor_verisi_yeterli" in q:
        doc = doctor_intel.get("doctor_kpi", pd.DataFrame()) if doctor_intel else pd.DataFrame()
        if doc is not None and not doc.empty:
            answers.append(f"Doktor verisi okunuyor. Seçili dönemde {len(doc)} doktor/hekime ait kayıt analiz edildi.")
            answers.append("Not: Mevcut dosyalarda doktor-ilaç kalem bağlantısı yoksa doktorun yazdığı ilaçlar kesin çıkarılamaz; sadece doktor bazlı ciro/reçete görünür.")
        else:
            answers.append("Doktor alanı boş veya okunamadı. Bu durumda doktor bazlı analiz güvenilir üretilemez.")

    elif "kurum_bazinda_en_yuksek_ciro" in q:
        if kurum is not None and not kurum.empty and "ciro" in kurum.columns:
            r = kurum.sort_values("ciro", ascending=False).iloc[0]
            answers.append(f"En yüksek ciro getiren kurum: {r.get('kurum','-')} · {money_fmt(r.get('ciro',0))}.")
            answers.append(f"Kurum sayısı: {kurum.get('kurum', pd.Series(dtype=str)).nunique()}.")
        else:
            answers.append("Kurum bazlı cevap için satış hareketlerinde kurum alanı bulunmalı.")

    elif "kurum_bazinda_tahsilat_acigi" in q:
        if kurum is not None and not kurum.empty and "tahsilat_acigi" in kurum.columns:
            gap = kurum.sort_values("tahsilat_acigi", ascending=False).iloc[0]
            answers.append(f"Kurum bazında en yüksek tahsilat açığı: {gap.get('kurum','-')} · {money_fmt(gap.get('tahsilat_acigi',0))}.")
        else:
            answers.append("Kurum bazlı tahsilat açığı için kurum ve tahsilat alanları okunmalı.")

    elif "tahsilat_yapim" in q or "tahsilat_acigi" in q:
    ...
    elif "finansal_ilk_aksiyon" in q:
        if current_stats.get("tahsilat_acigi", 0) > 0:
            answers.append(f"İlk finansal aksiyon: tahsilat açığını kontrol et · {money_fmt(current_stats.get('tahsilat_acigi',0))}.")
        elif current_stats.get("marj", 0) < 0.15:
            answers.append(f"İlk finansal aksiyon: marjı düşük ürünleri kontrol et · mevcut marj {pct_fmt(current_stats.get('marj',0))}.")
        else:
            answers.append("Finansal ilk aksiyon: sağlıklı marjı koruyarak sipariş bütçesini ve stok devir hızını izlemek.")

    elif "kki_riskim" in q:
        kki = risk_sets["kki"]
        answers.append(f"KKİ/riskli ürün eşleşmesi: {len(kki)} ürün.")
        if not kki.empty:
            answers.append("İlk kontrol: " + _product_list(kki, "satis_tutari", 3) + ".")
            if "kki_tahmini_fark_tl" in kki.columns:
                answers.append(f"Tahmini KKİ fark etkisi: {money_fmt(kki.get('kki_tahmini_fark_tl', pd.Series(dtype=float)).sum())}.")
        else:
            answers.append("Mevcut Risk Master eşleşmesine göre KKİ ürünü görünmüyor.")

    elif "recete_risk_durumu" in q:
        red_count = len(risk_sets["kirmizi"])
        green_count = len(risk_sets["yesil"])
        mor_count = len(risk_sets["mor"])
        ek_count = len(risk_sets["ek_izlem"])
        answers.append(f"Kırmızı: {red_count} · Yeşil: {green_count} · Mor: {mor_count} · Ek İzlem: {ek_count} ürün.")
        if mor_count == 0 and "mor_recete_mi" not in pm.columns:
            answers.append("Mor reçete için ayrı master alanı eklenmemişse bu sayı 0 görünür; Raporlar bölümündeki Risk Master PDF'i eczacı kontrolü için kullanılmalı.")
        if red_count:
            answers.append("Kırmızı reçetede ilk kontrol: " + _product_list(risk_sets["kirmizi"], "satis_tutari", 3) + ".")

    else:
        answers.append("Bu hazır soru mevcut veri alanlarıyla birebir eşleşmedi. Lütfen hazır analiz kartlarından birini seçin.")
        answers.append("Serbest soru modu gerçek LLM/API bağlantısı eklendiğinde açılmalı.")
...
if page == "🏠 Sabah Brifingi":
    ...
    with copilot_tabs[1]:
        ...
        for ans in copilot_answer(selected_q, product_master, current_stats, reorder_df, dead_df, slow_df, business_insights, patient_loyalty, doctor_intel, payment_df, kurum_df):
